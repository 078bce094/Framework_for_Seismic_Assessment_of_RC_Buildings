...
# IM_data = pd.read_csv('your_file.txt', delim_whitespace=True, header=None)
# GM_RSN_data = IM_data.iloc[:, 1]  # Get the second column

# # To print a specific record
# print(GM_RSN_data[0])  # Should print something like RSN14_KE
...
def compute_mean_period(accel, dt):
    # Compute FFT and frequencies
    n = len(accel)
    fft_amp = np.abs(np.fft.fft(accel))  # Fourier amplitudes
    freqs = np.fft.fftfreq(n, d=dt)  # Frequencies (Hz)

    # Remove DC component and negative frequencies
    mask = freqs > 0
    freqs = freqs[mask]
    periods = 1.0 / freqs  # Convert frequency to period (s)
    fft_amp = fft_amp[mask]

    # Compute weighted average (Rathje et al., 1998)
    numerator = np.sum(fft_amp**2 * periods)
    denominator = np.sum(fft_amp**2)
...
if u == 128:
    ...
    for CarlSagan in range(1):
        ...
        # Read acceleration data from .dat file
        with open(GM_output_file, "r") as f:
            lines = f.readlines()
        ...
        # Parse acceleration values (convert scientific notation)
        load_factors = []
        for line in lines:
            values = line.strip().split()
            load_factors.extend([float(val) for val in values])
        load_factors = np.array(load_factors)  # Convert to numpy array
        ...
        eps1C = eps1U * ( 1 + 5 * ( fc1C / fc1U - 1 ) )     # strain at maximum compressive stress 
        ...
        T1 = periods[1]   # Fundamental period of the structure (s)

        # # Compute mean period (Tm)
        # Tm = compute_mean_period(load_factors, dt)
        # print(f"Mean Period (Tm): {Tm:.4f} s")
        ...
        # if direction == 1:
        #     phiy, phiu, yieldM, ultM = MomentCurvature3D(Col_1_SecTag_Fiber, P, Col_1_y, Col_1_Cover, mu, numIncr, 'y')
        # elif direction == 2:
        #     phiy, phiu, yieldM, ultM = MomentCurvature3D(Col_1_SecTag_Fiber, P, Col_1_z, Col_1_Cover, mu, numIncr, 'z')
        # else:
        #     print("ERROR Direction.")
