...
def Section_Builder ():
    ...
    aBI_Beam_1 = area(12.0)         # area of mid layer bars || to local y axis
    aBB_Beam_1 = area(12.0)         # area of bottom layer bars || to local z axis
    nfCore_y_Beam_1 = 8      # number of fibers in the core patch in local y axis
    nfCore_z_Beam_1 = 8      # number of fibers in the core patch in local z axis
    nfCover_y_Beam_1 = 8     # number of fibers in the cover patches with long sides || to local y axis
    nfCover_z_Beam_1 = 8     # number of fibers in the cover patches with long sides || to local z axis

    # Beam Type 2
    Beam_2_sec_name = 'Beam Type 2 Section'
    nBT_Beam_2 = 2       # no. of longitudinal-reinforcement bars on top layer || to local z axis
    ...
    nBB_Beam_2 = 2       # no. of longitudinal-reinforcement bars on bottom layer || to local z axis
    aBT_Beam_2 = area(16.0)         # area of top layer bars || to local z axis
    aBM_Beam_2 = area(12.0)         # area of mid layer bars || to local z axis
    aBI_Beam_2 = area(16.0)         # area of mid layer bars || to local y axis
    aBB_Beam_2 = area(12.0)         # area of bottom layer bars || to local z axis
    nfCore_y_Beam_2 = 8      # number of fibers in the core patch in local y axis
    nfCore_z_Beam_2 = 8      # number of fibers in the core patch in local z axis
    nfCover_y_Beam_2 = 8     # number of fibers in the cover patches with long sides || to local y axis
    nfCover_z_Beam_2 = 8     # number of fibers in the cover patches with long sides || to local z axis
    ...
    nfCore_y_Col_1 = 8      # number of fibers in the core patch in local y axis
    nfCore_z_Col_1 = 8      # number of fibers in the core patch in local z axis
    nfCover_y_Col_1 = 8     # number of fibers in the cover patches with long sides || to local y axis
    nfCover_z_Col_1 = 8     # number of fibers in the cover patches with long sides || to local z axis
    ...
    nfCore_y_Col_2 = 8      # number of fibers in the core patch in local y axis
    nfCore_z_Col_2 = 8      # number of fibers in the core patch in local z axis
    nfCover_y_Col_2 = 8     # number of fibers in the cover patches with long sides || to local y axis
    nfCover_z_Col_2 = 8     # number of fibers in the cover patches with long sides || to local z axis
        ...
        if startY in (1, NplaneY):
            for k in range(NplaneZ - 1):
                ...
                for i in range(NplaneX - 1):
                    ...
                    ops.element('Truss', strutTag1, *[startNode1, endNode1], strut_areaX, hys_Mat_tag_X)
                    ops.element('Truss', strutTag2, *[startNode2, endNode2], strut_areaX, hys_Mat_tag_X)

def infill_along_Y():
    for i in range(NplaneY):
        startX = i + 1
        endX = i + 1
...
elif infillIncusion == 1:
    print('Infills applied along X Direction...')
    infill_along_X()    # if time history is done in X direction
elif infillIncusion == 2:
    print('Infills applied along Y Direction...')
    infill_along_Y()  # if time history is done in Y direction
elif infillIncusion == 3:
    print('Infills applied along both X and Y Direction...')
    infill_along_X()
    infill_along_Y()
else:
...
LL = 2.0e-3                                # Live load for all floors N per mm2
...
if bay_width_Y/bay_width_X <= 2.0 :
    ...
    P2 = 0
# --------------------------------- 
O_YBeam = P1 / g   + (0.7 * 20.0e-6 * bay_width_Y * bay_width_Z * 230) / g      # External Load on Outside Y Beam in mass terms : N s2 / mm
I_YBeam = 2 * P1 / g   # External Load on Inside Y Beam in mass terms : N s2 / mm

O_XBeam = P2 / g + (0.7 * 20.0e-6 * bay_width_X * bay_width_Z * 230) / g       # External Load on Outside X Beam in mass terms : N s2 / mm
