...
nbc_205_1994_dir = (base_dir / "Groups" / "NBC_205_1994")
if not nbc_205_1994_dir.exists():
    print('error getting nbc_205_1994_dir')
...
samples_data_file = nbc_205_1994_dir / "NBC_205_1994_samples.csv"
...
output_csv_file = nbc_205_1994_dir / "NBC_205_1994_results.csv"
...
comp_samples_csv_file = nbc_205_1994_dir / "samples_completed.csv"
...
def run_analysis(u, CarlSagan):
    ...
    Analysis_output_dir = nbc_205_1994_dir / "Output" / f"sample_{sample_no}"
    ...
    if direction == 2:
        ...
        if NBayZ == 3:
            ...
            materials_function()
            Section_Builder()
            phiy[2], phiu[2], yieldM[2], ultM[2] = MomentCurvature3D(Col_2_SecTag, P, Col_2_z, Col_2_Cover, mu, numIncr, 'z', Fy_steel, E0_steel)
    else:
        print("ERROR Direction.")

    input_dir = (nbc_205_1994_dir / "Output" / f"sample_{sample_no}" / f"Earthquake_{CarlSagan}")
    ...
    if sum(deno_per_floor_park) > 0:
        all_floor_Park =  round((((sum(nume_per_floor_park)) / (sum(deno_per_floor_park))) * 100),3)  # all storey combined
        if all_floor_Park > 100:
            output_array.append(100.0)  # all storey combined
        else:
            output_array.append(all_floor_Park)  # all storey combined
    else:
...
#     tasks = [(u, CarlSagan) for u in us for CarlSagan in range(1)]
